...
(defroutes web-handler
  (refreshable 'set.routes/handler)
...
(defn app-handler []
  (if config/development?
    (let [wrap-verbose    (util/resolve-var 'c3kit.apron.verbose/wrap-verbose)
      ...
      (-> (refresh-handler 'set.http/web-handler)
          wrap-verbose))
    (util/resolve-var 'set.http/web-handler)))
...
;;  1) When the app starts, some namespaces are loaded, like this one.  But the refresh code (set.refresh)
;;      doesn't know which. As far as it knows, nothing has been loaded.  So on the first request, all the namespaces
;;      are reloaded.
;;  2) Some namespaces will/should never get reloaded. See set.refresh/excludes
...
;;        - set.routes/handler - the essence of set.com
...
(defonce root-handler
  (-> (app-handler)
      ...
      (wrap-jwt {:cookie-name "set-token" :secret (:jwt-secret config/env) :lifespan (when config/development? (time/hours 336))})
      wrap-keyword-params
