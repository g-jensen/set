...
(def default-title "set")
(def default-description "The set App")
           ...
           [:head
            ...
            (if config/development?
              (list
                ...
                (page/include-js "/cljs/set_dev.js"))
              (page/include-js (add-fingerprint "/cljs/set.js")))
            (:head options)                                 ;; MDM - must go after js so we can include js-fns, and before css, so we can override styles as needed
            (page/include-css (add-fingerprint (or (:css options) "/css/set.css")))]
           [:body body]))
  ...
  ([data]
   (let [payload (pr-str (utilc/->transit data))]
     (str "<script type=\"text/javascript\">\n//<![CDATA[\n"
          "set.main.main(" (str/replace payload "</script>" "<\\/script>") ");"
          "\n//]]>\n</script>"))))
...
(defn rich-client [payload options]
  (default (list [:div#app-root rich-client-placeholder]
                 (client-init payload))
           (assoc options
             :head (elem/javascript-tag (str "goog.require('set.main');")))))
...
(defn build-rich-client-payload [request]
  {:flash  (flash/messages request)
   :config {
            ...
            :set-root          (-> config/env :cleancoders-auth :url-root)
            :host               config/host
            }})
